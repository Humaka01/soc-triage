...
async function proxiedFetch(url, opts = {}) {
  ...
  throw new Error("All proxies failed — check network connectivity");
}
...
elModalGo.addEventListener("click", () => {
  ...
  if (!vt && !abuse) {
    toast("Enter at least one API key.", "err");
    return;
  }
  KEYS.vt = vt;
  KEYS.abuse = abuse;
...
async function cfFetch(domain) {
  ...
  const res = await proxiedFetch(
    `https://api.cloudflare.com/client/v4/radar/domains/top?domain=${encodeURIComponent(domain)}&limit=1`,
    {
      headers: {
        Authorization: `Bearer ${KEYS.cf}`,
