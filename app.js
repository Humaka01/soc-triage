...
const KEYS = { vt: '', abuse: '', cf: '' };

// ── Direct API fetch (no proxy needed from https:// origin) ──
...
// ── State ──────────────────────────────────────────
const S = {
  tickets: [], allIps: [], allUrls: [], allDomains: [], allHashes: [],
  activeTicket: null, activeTab: 'tickets',
...
const OSINT = {
  ip: [
    { l:'VirusTotal',   u: v=>`https://www.virustotal.com/gui/ip-address/${v}/details` },
    { l:'AbuseIPDB',    u: v=>`https://www.abuseipdb.com/check/${v}` },
    { l:'Talos',        u: v=>`https://talosintelligence.com/reputation_center/lookup?search=${v}` },
    { l:'CrowdSec',     u: v=>`https://app.crowdsec.net/analysis/ip/${v}` },
    { l:'Shodan',       u: v=>`https://www.shodan.io/host/${v}` },
    { l:'IPQualityScore',u:v=>`https://www.ipqualityscore.com/free-ip-lookup/${v}` },
  ],
  url: [
    { l:'VirusTotal',   u: v=>`https://www.virustotal.com/gui/url/${encodeURIComponent(v)}/detection` },
    { l:'URLScan',      u: v=>`https://urlscan.io/search/#${encodeURIComponent(v)}` },
    { l:'Talos',        u: v=>`https://talosintelligence.com/reputation_center/lookup?search=${encodeURIComponent(v)}` },
    { l:'Kaspersky',    u: v=>`https://opentip.kaspersky.com/${encodeURIComponent(v)}` },
  ],
  domain: [
    { l:'VirusTotal',   u: v=>`https://www.virustotal.com/gui/domain/${v}/details` },
    { l:'URLScan',      u: v=>`https://urlscan.io/search/#${encodeURIComponent(v)}` },
    { l:'Talos',        u: v=>`https://talosintelligence.com/reputation_center/lookup?search=${v}` },
  ...
  hash: [
    { l:'VirusTotal',   u: v=>`https://www.virustotal.com/gui/file/${v}/detection` },
    { l:'Kaspersky',    u: v=>`https://opentip.kaspersky.com/${v}` },
    { l:'MalwareBazaar',u: v=>`https://bazaar.abuse.ch/browse.php?search=sha256%3A${v}` },
  ],
};

// ── Trusted domains ────────────────────────────────
const TRUSTED = new Set([
  'microsoft.com','outlook.com','protection.outlook.com','office.com','office365.com',
  'microsoftonline.com','live.com','azure.com','azure-api.net','mails.microsoft.com',
  'cdn-dynmedia-1.microsoft.com','windows.net',
  'google.com','googleapis.com','gstatic.com','gmail.com','googleusercontent.com',
  'apple.com','icloud.com','amazon.com','amazonaws.com','cloudfront.net',
  'github.com','github.io','slack.com','zoom.us','dropbox.com',
...
elModalGo.addEventListener('click', () => {
  ...
  const cf    = elKeyCF.value.trim();
  if (!vt && !abuse) { toast('Enter at least one API key.', 'err'); return; }
  KEYS.vt    = vt;
  KEYS.abuse = abuse;
  KEYS.cf    = cf;
  closeModal();
...
function switchTab(tab) {
  ...
  document.querySelectorAll('.ctr').forEach(c => c.classList.toggle('active', c.dataset.tab === tab));
  S.activeTab = tab;
}

document.querySelectorAll('.snav').forEach(b => b.addEventListener('click', () => switchTab(b.dataset.tab)));
document.querySelectorAll('.ctr').forEach(c => c.addEventListener('click', () => switchTab(c.dataset.tab)));

// ── Filters ────────────────────────────────────────
elSearchInput.addEventListener('input',    () => { S.filters.q        = elSearchInput.value.toLowerCase(); renderTickets(); });
elFStatus.addEventListener('change',       () => { S.filters.status   = elFStatus.value;   renderTickets(); });
elFPriority.addEventListener('change',     () => { S.filters.priority = elFPriority.value; renderTickets(); });
elFSeverity.addEventListener('change',     () => { S.filters.severity = elFSeverity.value; renderTickets(); });
...
elManualClear.addEventListener('click', () => { elManualText.value = ''; });
elManualProc.addEventListener('click', () => {
  const raw = elManualText.value.trim();
  if (!raw) { toast('Enter at least one IOC.', 'warn'); return; }
...
function parseCSVRows(text) {
  ...
  const rows = []; let inQ=false, cur='', cols=[];
...
function processCSV(raw) {
  ...
  if (rows.length < 2) { toast('CSV is empty or invalid.', 'err'); return; }
  ...
  for (let i=1; i<rows.length; i++) {
    ...
    const tiDoms   = get('Custom field (Threat Intelligence | Domain Indicators)');
    ...
    const tactics = rawTactics
      ? rawTactics.replace(/['"]/g,'').split(',').map(t=>t.trim()).filter(Boolean)
      : [];
  ...
  if (!tickets.length) { toast('No valid tickets found in CSV.', 'err'); return; }
...
function processText(text, typeHint='auto') {
  ...
  const urls    = typeHint==='url'    ? parseManual(text,'url')    : extractURLs(text);
  const domains = typeHint==='domain' ? parseManual(text,'domain') : extractDomains(text, urls);
  const hashes  = typeHint==='hash'   ? parseManual(text,'hash')   : extractHashes(text);
  ...
  if (!S.tickets.length) {
    ingestTickets([{
      key: 'MANUAL-1', summary: 'Manual Input', issueType: 'Manual',
      status:'', priority:'', assignee:'', reporter:'', created:'', updated:'',
      resolved:'', description: text, sevSOC:'', sevDEV:'', tactics:[],
      sentinelARM:'', sentinelTitle:'', summaryInv:'', procLink:'',
      requestType:'', statusCat:'',
      ips, urls, domains, hashes,
    }]);
  } else {
    ...
    const add = (arr, newItems, ticketKey) => {
      newItems.forEach(v => {
        ...
        if (ex) { if (!ex.tickets.includes(ticketKey)) ex.tickets.push(ticketKey); }
...
function parseManual(text, type) {
  ...
  if (type==='url')    return [...new Set(items.filter(s=>s.startsWith('http')))];
  if (type==='domain') return [...new Set(items.filter(s=>/^([a-z0-9-]+\.)+[a-z]{2,}$/i.test(s)))];
  if (type==='hash')   return [...new Set(items.filter(s=>/^[A-Fa-f0-9]{32,128}$/.test(s)))];
  return [];
}

function ingestTickets(tickets) {
  S.tickets    = [...S.tickets, ...tickets];
  S.allIps     = dedup([...S.allIps,     ...tickets.flatMap(t=>t.ips.map(v=>({value:v,ticketKey:t.key})))]);
  S.allUrls    = dedup([...S.allUrls,    ...tickets.flatMap(t=>t.urls.map(v=>({value:v,ticketKey:t.key})))]);
  S.allDomains = dedup([...S.allDomains, ...tickets.flatMap(t=>t.domains.map(v=>({value:v,ticketKey:t.key})))]);
  S.allHashes  = dedup([...S.allHashes,  ...tickets.flatMap(t=>t.hashes.map(v=>({value:v,ticketKey:t.key})))]);
...
function isValidIP(ip) { return ip.split('.').every(n=>+n>=0&&+n<=255); }
...
function extractDomains(text, urls) {
  const urlHosts = urls.map(u=>{ try{return new URL(u).hostname.toLowerCase();}catch{return null;} }).filter(Boolean);
  const re = /\b([a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z]{2,}\b/gi;
  const plain = (text.match(re)||[]).map(d=>d.toLowerCase())
...
function extractHashes(text) {
  const pats=[/\b[A-Fa-f0-9]{128}\b/g,/\b[A-Fa-f0-9]{64}\b/g,/\b[A-Fa-f0-9]{40}\b/g,/\b[A-Fa-f0-9]{32}\b/g];
  const r=new Set();
...
function dedup(arr) {
  ...
  arr.forEach(item=>{
    ...
    if (ticketKey&&!m.get(key).tickets.includes(ticketKey)) m.get(key).tickets.push(ticketKey);
  });
  return [...m.values()];
}
...
function filteredTickets() {
  ...
  return S.tickets.filter(t=>{
    ...
    if (q) {
      const blob=[t.key,t.summary,t.description,t.assignee,...t.ips,...t.urls,...t.domains,...t.hashes,...t.tactics].join(' ').toLowerCase();
...
function renderTickets() {
  ...
  elTicketList.querySelectorAll('.tkt').forEach(card=>{
    card.addEventListener('click',()=>{
      const t=S.tickets.find(x=>x.key===card.dataset.key); if(!t)return;
      elTicketList.querySelectorAll('.tkt').forEach(c=>c.classList.remove('sel'));
...
function buildTicketCard(t) {
  const total = t.ips.length+t.urls.length+t.domains.length+t.hashes.length;
  const isActive = S.activeTicket===t.key;
  const sevCls = {High:'thr-high',Critical:'thr-high',Medium:'thr-med'}[t.sevSOC]||'';

  const chips = [
    ...t.ips.slice(0,2).map(v=>`<span class="chip ip">${esc(v)}</span>`),
    ...t.domains.slice(0,2).map(v=>`<span class="chip domain">${esc(v)}</span>`),
    ...t.urls.slice(0,1).map(v=>`<span class="chip url">${esc(trunc(v,38))}</span>`),
    ...t.hashes.slice(0,1).map(v=>`<span class="chip hash">${esc(v.slice(0,12))}…</span>`),
  ];
  const extra = total - chips.length;
  if (extra>0) chips.push(`<span class="chip more">+${extra} more</span>`);

  const verdictRows = t.ips.slice(0,2).map(ip=>{
  ...
  const tactics = t.tactics.slice(0,3).map(tac=>`<span class="tactic">${esc(tac)}</span>`).join('');
...
function renderDetailPane(t) {
  ...
  const fieldHtml = fields.map(([lbl,val])=>{
    let disp=esc(val);
    if (lbl==='Procedure'&&val.startsWith('http')) disp=`<a href="${esc(val)}" target="_blank" rel="noreferrer">Open ↗</a>`;
    if (lbl==='Sentinel ARM') {
  ...
  const descClean=(t.description||'Nu există descriere.')
    .replace(/\[Link\|([^\]]+)\]/g,'$1').replace(/\*\s+/g,'• ').replace(/\\n/g,'\n');
...
function buildIOCGrid(items, type) {
  if (!items.length) return `<div class="splash"><div class="splash-mark">◈</div><div class="splash-title">No ${type} indicators</div></div>`;
  return items.map(item=>{
    const {value:val, tickets:refs}=item;
    const trusted=(type==='domain'||type==='url')&&isTrusted(type==='url'?(()=>{try{return new URL(val).hostname;}catch{return val;}})():val);
...
function renderAnalytics() {
  const t=S.tickets;
  if (!t.length) { elAnalyticsGrid.innerHTML=`<div class="splash"><div class="splash-mark">◈</div><div class="splash-title">Import data to see analytics</div></div>`; return; }
  const cnt=(arr,key)=>{ const m={}; arr.forEach(x=>{const v=x[key]||'Unknown';m[v]=(m[v]||0)+1;}); return Object.entries(m).sort((a,b)=>b[1]-a[1]); };
  const total=S.allIps.length+S.allUrls.length+S.allDomains.length+S.allHashes.length;
  const withIOC=t.filter(x=>x.ips.length+x.urls.length+x.domains.length+x.hashes.length>0).length;
  const tacMap={}; t.flatMap(x=>x.tactics).forEach(tac=>{tacMap[tac]=(tacMap[tac]||0)+1;});
  ...
  elAnalyticsGrid.innerHTML=`
    ...
    <div class="an-card"><div class="an-title">IOC Breakdown</div>${bc([['IPs',S.allIps.length],['Domains',S.allDomains.length],['URLs',S.allUrls.length],['Hashes',S.allHashes.length]],total,{IPs:'var(--cyan)',Domains:'var(--yellow)',URLs:'var(--orange)',Hashes:'var(--purple)'})}</div>
...
function bc(entries,max,cmap) {
  if (!entries.length||!max) return '<div style="color:var(--t4);font-size:11px">—</div>';
  return entries.map(([lbl,n])=>`
    <div class="bar-row">
...
function statusBadge(s) {
  const m={Open:'b-open',Reopened:'b-reopened',Closed:'b-closed',Resolved:'b-closed'};
...
function verdictBadge(result) {
  ...
  if (result.err) return `<span class="verd v-err" title="${esc(result.err)}">Error</span>`;
  const parts=[];
  if (result.vt) {
    const {malicious:m,suspicious:s,total:tot}=result.vt;
    const f=m+s;
    const cls=m>=5?'v-hi':m>=1||s>=3?'v-med':'v-clean';
    parts.push(`<span class="verd ${cls}" title="VirusTotal: ${f}/${tot} vendors">VT ${f}/${tot}</span>`);
  }
  if (result.abuse) {
    const sc=result.abuse.score;
    const cls=sc>=75?'v-hi':sc>=25?'v-med':'v-clean';
    const tip=`AbuseIPDB: ${sc}% · ${result.abuse.isp||''} · ${result.abuse.country||''}`.replace(/· ·/g,'·').trim();
    parts.push(`<span class="verd ${cls}" title="${esc(tip)}">Abuse ${sc}%</span>`);
  }
  if (result.cf&&result.cf.cats) {
    parts.push(`<span class="verd v-med" title="Cloudflare Radar">CF: ${esc(result.cf.cats)}</span>`);
  }
  return parts.length?parts.join(''):`<span class="verd v-queue">No data</span>`;
}
...
function enqueue(fn) { return new Promise((res,rej)=>{ apiQueue.push({fn,res,rej}); drain(); }); }
...
async function vtFetch(url) {
  ...
  const res=await proxiedFetch(url,{headers:{'x-apikey':KEYS.vt}});
...
async function abuseFetch(ip) {
  ...
  const res=await proxiedFetch(`https://api.abuseipdb.com/api/v2/check?ipAddress=${ip}&maxAgeInDays=90&verbose`,{
    headers:{'Key':KEYS.abuse,'Accept':'application/json'}
  });
...
async function cfFetch(domain) {
  if (!KEYS.cf) return null;
  const res=await proxiedFetch(`https://api.cloudflare.com/client/v4/radar/domains/top?domain=${encodeURIComponent(domain)}&limit=1`,{
...
function parseVT(data) {
  ...
  return {malicious:s.malicious||0,suspicious:s.suspicious||0,total:Object.values(s).reduce((a,b)=>a+b,0)};
}
function parseAbuse(data) {
  if (!data?.data) return null;
  return {score:data.data.abuseConfidenceScore,isp:data.data.isp||'',country:data.data.countryCode||'',reports:data.data.totalReports||0};
}
function parseCF(data) {
  if (!data?.result) return null;
  const cats=(data.result.categories||[]).map(c=>c.name||c).join(', ');
...
function setRunning(v) {
  ...
  if (!v) { elProgFill.style.width='0%'; elProgText.textContent=''; }
}
...
async function startAnalysis() {
  ...
  const iocs=[
    ...S.allIps.filter(i=>!S.cache[i.value]&&!S.pending.has(i.value)).map(i=>({value:i.value,type:'ip'})),
    ...S.allDomains.filter(d=>!isTrusted(d.value)&&!S.cache[d.value]&&!S.pending.has(d.value)).map(d=>({value:d.value,type:'domain'})),
    ...S.allHashes.filter(h=>!S.cache[h.value]&&!S.pending.has(h.value)).map(h=>({value:h.value,type:'hash'})),
  ];
  ...
  for (const ioc of iocs) {
    ...
    await enqueue(async()=>{
      ...
      try {
        if (ioc.type==='ip') {
          const [vt,ab]=await Promise.allSettled([
            vtFetch(`https://www.virustotal.com/api/v3/ip_addresses/${ioc.value}`),
            abuseFetch(ioc.value),
          ]);
          if (vt.status==='fulfilled') r.vt=parseVT(vt.value);
          if (ab.status==='fulfilled') r.abuse=parseAbuse(ab.value);
        } else if (ioc.type==='domain') {
          const [vt,cf]=await Promise.allSettled([
            vtFetch(`https://www.virustotal.com/api/v3/domains/${ioc.value}`),
            cfFetch(ioc.value),
          ]);
          if (vt.status==='fulfilled') r.vt=parseVT(vt.value);
          if (cf.status==='fulfilled') r.cf=parseCF(cf.value);
        } else if (ioc.type==='hash') {
          const vt=await vtFetch(`https://www.virustotal.com/api/v3/files/${ioc.value}`);
          r.vt=parseVT(vt);
        }
      } catch(e) {
        r.err=e.message;
      ...
      if (S.activeTicket) {
        ...
        if (active&&[...active.ips,...active.domains,...active.hashes].includes(ioc.value)) renderDetailPane(active);
      }
    });
  }
  ...
  setRunning(false);
  if (!S.aborted) {
    toast(`Analysis complete — ${done} IOC${done!==1?'s':''} checked.`,'ok');
...
// ── Reset ──────────────────────────────────────────
function resetAll() {
  if (S.running) { S.aborted=true; apiQueue.length=0; setRunning(false); }
  Object.assign(S,{tickets:[],allIps:[],allUrls:[],allDomains:[],allHashes:[],activeTicket:null,cache:{},pending:new Set(),aborted:false,running:false});
  ...
  elIpList.innerHTML=elDomainList.innerHTML=elUrlList.innerHTML=elHashList.innerHTML=`<div class="splash"><div class="splash-mark">◈</div><div class="splash-title">No indicators</div></div>`;
